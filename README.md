# Nombre del laboratorio

<p align="center">
  <img src="Assets/Landing.png" alt="Portada del Proyecto" width="100%">
</p>

Aqui voy a poner una breve explicación mi linea y mis colegas


<p align="center">
  <a href="Team.md" style="text-decoration: none; display: inline-block; margin: 15px;">
    <img src="Assets/Team.png" alt="Team" 
         style="width: 150px; height: 150px; border-radius: 100%; object-fit: cover; border: 3px solid #0366d6; display: block; margin: 0 auto;">
    <span style="display: block; text-align: center; color: #0366d6; font-weight: bold; margin-top: 8px; font-family: sans-serif;">Team</span>
  </a>

  <a href="proyectos.md" style="text-decoration: none; display: inline-block; margin: 15px;">
    <img src="Assets/Projects.png" alt="Projects" 
         style="width: 150px; height: 150px; border-radius: 100%; object-fit: cover; border: 3px solid #0366d6; display: block; margin: 0 auto;">
    <span style="display: block; text-align: center; color: #0366d6; font-weight: bold; margin-top: 8px; font-family: sans-serif;">Projects</span>
  </a>

  <a href="tutoriales.md" style="text-decoration: none; display: inline-block; margin: 15px;">
    <img src="Assets/Tutorials.png" alt="Tutorials" 
         style="width: 150px; height: 150px; border-radius: 100%; object-fit: cover; border: 3px solid #0366d6; display: block; margin: 0 auto;">
    <span style="display: block; text-align: center; color: #0366d6; font-weight: bold; margin-top: 8px; font-family: sans-serif;">Tutoriales</span>
  </a>
</p>
